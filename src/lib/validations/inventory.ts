...
export const maternityDressSchema = z.object({
  ...
  price: z.number().min(0.01, 'Price must be greater than 0').max(9999.99, 'Price is too high'),
...
export const maternityDressVariantSchema = z.object({
  maternityDressId: z.string().min(1, 'Maternity dress ID is required'),
...
export const inventoryProductSchema = z.object({
  ...
  sellingPrice: z.number().min(0.01, 'Selling price must be greater than 0').max(9999.99, 'Selling price is too high'),

  costPrice: z.number().min(0, 'Cost price cannot be negative').max(9999.99, 'Cost price is too high'),
  ...
  material: z.string().max(50, 'Material must not exceed 50 characters').optional(),

  occasion: z.string().max(50, 'Occasion must not exceed 50 characters').optional(),

  linkedTemplate: z.string().optional()
})
