...
function CategoryStylesSection({ categoryId }: { categoryId: string }) {
  ...
  const {
    ...
    error: stylesError
  } = useStylesByCategory(categoryId, {
    index: 1,
    pageSize: 10,
          ...
          <div className='flex items-start justify-between'>
            <div>
              ...
              <div className='flex items-center gap-3 text-sm text-gray-600'>
                <span>
                  Tổng: <span className='font-semibold text-blue-600'>{totalStyles} styles</span>
                </span>
                <span>•</span>
                <span>
                  Có hình ảnh: <span className='font-semibold text-green-600'>{hasImages}</span>
                </span>
              </div>
            </div>
          ...
          <CardContent className='p-6'>
              ...
              <div className='mb-6 p-4 border-2 border-dashed border-blue-200 rounded-lg bg-blue-50/30'>
                ...
                <form onSubmit={handleSubmitStyle} className='space-y-4'>
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                    <div>
                      <label className='block text-sm font-medium text-gray-700 mb-2'>Tên Style *</label>
                      <Input
                        value={formData.name}
            ...
            {stylesLoading ? (
              ...
              <div className='text-center py-12'>
                ...
                <p className='text-gray-500'>Danh mục này chưa có style nào được tạo</p>
              </div>
            ) : (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {styles.map((style) => (
                  <Card
                    key={style.id}
                    className='relative group hover:shadow-lg transition-shadow border border-gray-200'
                  >
                    <CardContent className='p-0'>
                      {style.image && style.image.length > 0 ? (
                        <CategoryImage src={style.image[0]} alt={style.name} count={style.image.length} />
                      ) : (
    ...
    <div className='space-y-6 py-4 min-h-screen'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground mt-1'>Quản lý các danh mục sản phẩm đầm bầu</p>
        </div>
        <div className='flex items-center gap-3'>
          <Button onClick={handleAddCategory} className='bg-blue-600 hover:bg-blue-700 text-white shadow-sm'>
            <Plus className='h-4 w-4 mr-2' />
            Thêm Danh Mục
          </Button>
        </div>
