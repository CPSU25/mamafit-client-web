...
export const WarrantyRequestDetail = ({ request, onClose }: WarrantyRequestDetailProps) => {
  ...
  useEffect(() => {
    if (warrantyItem?.items && Array.isArray(warrantyItem.items)) {
      const initialDecisions = warrantyItem.items.reduce<Record<string, ItemDecision>>(
        (acc, item) => {
  ...
  const handleItemDecision = (itemId: string, status: StatusWarrantyRequestItem, destinationType?: 'FACTORY' | 'BRANCH') => {
  ...
  const handleSubmit = () => {
    ...
    const hasItemsToDecide = warrantyItem?.items?.some(item => 
            ...
            {warrantyItem?.items?.map((item) => (
              <div key={item.orderItemId} className="border rounded-lg p-4 space-y-4">
                <div className="flex items-start justify-between">
                  ...
                  <Badge 
                    variant="outline" 
                    className={getItemStatusColor(itemDecisions[item.orderItemId]?.status ?? StatusWarrantyRequestItem.PENDING)}
                  >
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-blue-700 font-medium">
                            Đã được duyệt và đang vận chuyển
                          </span>
                        </div>
                    ...
                    {item.status !== StatusWarrantyRequestItem.IN_TRANSIT && (
                        ...
                        <div className="flex gap-2 flex-wrap">
                            <Button
                              size="sm"
                            variant={itemDecisions[item.orderItemId]?.status === StatusWarrantyRequestItem.REJECTED ? "default" : "outline"}
                            onClick={() => handleItemDecision(item.orderItemId, StatusWarrantyRequestItem.REJECTED)}
                            className={itemDecisions[item.orderItemId]?.status === StatusWarrantyRequestItem.REJECTED 
                          ...
                          >
                            ...
                            <Button
                              size="sm"
                            variant={itemDecisions[item.orderItemId]?.status === StatusWarrantyRequestItem.APPROVED && itemDecisions[item.orderItemId]?.destinationType === 'BRANCH' ? "default" : "outline"}
                            onClick={() => handleItemDecision(item.orderItemId, StatusWarrantyRequestItem.APPROVED, 'BRANCH')}
                            className={itemDecisions[item.orderItemId]?.status === StatusWarrantyRequestItem.APPROVED && itemDecisions[item.orderItemId]?.destinationType === 'BRANCH'
                          ...
                          <Button
                            size="sm"
                            variant={itemDecisions[item.orderItemId]?.status === StatusWarrantyRequestItem.APPROVED && itemDecisions[item.orderItemId]?.destinationType === 'FACTORY' ? "default" : "outline"}
                            onClick={() => handleItemDecision(item.orderItemId, StatusWarrantyRequestItem.APPROVED, 'FACTORY')}
                            className={itemDecisions[item.orderItemId]?.status === StatusWarrantyRequestItem.APPROVED && itemDecisions[item.orderItemId]?.destinationType === 'FACTORY'
                        ...
                        {itemDecisions[item.orderItemId]?.status === StatusWarrantyRequestItem.REJECTED && (
                          ...
                          <div className="space-y-3 p-3 bg-green-50 rounded-lg">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              <div>
                                ...
                                <Input
                                  ...
                                  onChange={(e) => handleItemDetailChange(item.orderItemId, 'fee', Number(e.target.value) || 0)}
                                  className="mt-1"
                                />
                              </div>
                              <div>
                                ...
                                <Input
                                  type="number"
                                  placeholder="7"
                                  value={itemDecisions[item.orderItemId]?.estimateTime ? Math.ceil((new Date(itemDecisions[item.orderItemId]?.estimateTime || '').getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) : ''}
                                  onChange={(e) => {
                            ...
                            {itemDecisions[item.orderItemId]?.destinationType === 'BRANCH' && (
                              <div>
                                ...
                                <Select
                                  ...
                                  onValueChange={(value) => handleItemDetailChange(item.orderItemId, 'destinationBranchId', value)}
                                >
                            ...
                            <div className="text-sm text-green-700">
                              ...
                              Được chấp nhận xử lý tại: <strong>{itemDecisions[item.orderItemId]?.destinationType === 'FACTORY' ? 'Nhà xưởng' : 'Chi nhánh'}</strong>
                              {itemDecisions[item.orderItemId]?.destinationType === 'BRANCH' && itemDecisions[item.orderItemId]?.destinationBranchId && (
                                <>
                                  {' - '}
                                  <strong>
                                    {branchesData?.data?.items?.find(b => b.id === itemDecisions[item.orderItemId]?.destinationBranchId)?.name || 'Chi nhánh đã chọn'}
                                  </strong>
                                </>
                              )}
                            </div>
