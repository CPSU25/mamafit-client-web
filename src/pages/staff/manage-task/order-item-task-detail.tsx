...
export default function OrderItemDetailPage() {
  ...
  if (!orderItemId) {
    return (
      <div className='container mx-auto p-4 sm:p-8 text-center'>
        <h2 className='text-lg sm:text-xl font-semibold text-red-500'>Sản phẩm không hợp lệ</h2>
  ...
  if (isLoading) {
    return (
      <div className='container mx-auto p-4 md:p-8 space-y-4 md:space-y-6'>
    ...
    return (
      <div className='container mx-auto p-4 sm:p-8 text-center'>
        <div className='text-red-500 space-y-2'>
          <h2 className='text-lg sm:text-xl font-semibold'>Không thể tải thông tin sản phẩm trong đơn hàng</h2>
          <p className='text-sm sm:text-base'>Đã có lỗi xảy ra khi tải thông tin chi tiết.</p>
        </div>
        <Button onClick={() => navigate('/system/staff/manage-task')} className='mt-4'>
          Quay lại danh sách
        </Button>
        ...
        <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
          <div className='flex flex-col sm:flex-row sm:items-center gap-3 md:gap-4'>
            <Button
              ...
              className='flex items-center gap-2 hover:bg-blue-100 w-fit'
            >
              <ArrowLeft className='h-4 w-4' />
              <span className='hidden sm:inline'>Quay lại</span>
            </Button>
            <div>
              <h1 className='text-xl sm:text-2xl md:text-3xl font-bold text-gray-900'>Chi tiết Order Item</h1>
              <div className='flex flex-col sm:flex-row sm:items-center gap-2 md:gap-4 mt-2'>
                <Badge
                  variant='secondary'
                  className={`${getStatusColor(orderItemData.orderStatus, 'order')} text-xs font-medium px-3 md:px-4 py-1 md:py-2 bg-white/90 dark:bg-white/80 text-violet-800 shadow-sm w-fit`}
                >
                ...
                <div className='flex items-center gap-2 md:gap-4 text-xs md:text-sm text-muted-foreground'>
                  ...
                  <span>{overallProgress}% tiến độ</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className='relative flex justify-center sm:justify-end'>
            <div className='w-16 h-16 md:w-20 md:h-20 rounded-full bg-blue-100 flex items-center justify-center'>
              <div className='w-12 h-12 md:w-16 md:h-16 rounded-full bg-white flex items-center justify-center'>
                <span className='text-lg md:text-xl font-bold text-blue-600'>{overallProgress}%</span>
              </div>
            </div>
            <div
            ...
            <CardContent className='space-y-3 md:space-y-4'>
              <div className='aspect-square w-full rounded-xl overflow-hidden shadow-md'>
                <ProductImageViewer
                  src={displayImage}
                  alt={displayName}
                  containerClassName='aspect-square w-full rounded-lg border-2 border-violet-200 dark:border-violet-700'
                  imgClassName='px-2'
                  fit='contain'
                />
              </div>
              ...
              <div className='space-y-3 md:space-y-4'>
                <div className='text-center'>
                  <h3 className='font-bold text-lg md:text-xl text-gray-900 mb-2'>{displayName}</h3>
                  ...
                  {maternityDressDetail && !preset && (
                    <div className='mt-3 space-y-2'>
                      ...
                      <div className='text-xs md:text-sm text-muted-foreground'>Số lượng: {maternityDressDetail.quantity}</div>
                    </div>
                  )}
                </div>
                ...
                <div className='space-y-2 md:space-y-3 text-xs md:text-sm bg-gray-50 p-3 rounded-lg'>
                  <div className='flex items-center gap-2'>
                    <Package className='h-3 w-3 md:h-4 md:w-4 text-muted-foreground flex-shrink-0' />
                    <span className='font-medium'>Mã đơn hàng:</span>
                    <code className='text-xs bg-white px-2 py-1 rounded border break-all'>{orderCode}</code>
                  </div>
                  <div className='flex items-center gap-2'>
                    <Tag className='h-3 w-3 md:h-4 md:w-4 text-muted-foreground flex-shrink-0' />
                    <span className='font-medium'>Số lượng:</span>
                    <Badge variant='outline' className='text-xs'>{orderItemData.orderItem?.quantity || 1}</Badge>
                  </div>
                  <div className='flex items-center gap-2'>
              ...
              {orderItem?.addOnOptions && orderItem.addOnOptions.length > 0 ? (
                <div className='space-y-3 md:space-y-4'>
                  {orderItem.addOnOptions.map((addon) => (
                    <div
                      key={addon.id}
                      className='bg-white p-3 md:p-4 rounded-lg border border-purple-200 shadow-md hover:shadow-lg transition-shadow duration-200'
                    >
                      <div className='flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 md:gap-3 mb-3'>
                        <div className='flex-1'>
                          <h4 className='font-bold text-gray-900 text-sm md:text-base mb-1'>{addon.name}</h4>
                          <p className='text-xs md:text-sm text-gray-600 leading-relaxed'>{addon.description}</p>
                        </div>
                        <Badge
                          variant='outline'
                          className='w-fit px-2 md:px-3 py-1 bg-green-50 text-green-700 border-green-200 font-semibold text-xs'
                        >
                      ...
                      <div className='space-y-2'>
                        <div className='flex flex-col sm:flex-row sm:items-center gap-1 md:gap-2'>
                          ...
                          <Badge
                            variant='secondary'
                            className='w-fit px-2 py-1 text-xs bg-purple-100 text-purple-700 border-purple-200'
                          >
                            {addon.position.name}
                          </Badge>
                        </div>
                        ...
                        <div className='flex flex-col sm:flex-row sm:items-center gap-1 md:gap-2'>
                          ...
                          <Badge
                            variant='secondary'
                            className='w-fit px-2 py-1 text-xs bg-blue-100 text-blue-700 border-blue-200'
                          >
                            {addon.size.name}
                          </Badge>
                        </div>
                        ...
                        <div className='flex flex-col sm:flex-row sm:items-center gap-1 md:gap-2'>
                          ...
                          <Badge
                            variant='outline'
                            className='w-fit px-2 py-1 text-xs bg-gray-100 text-gray-700 border-gray-200'
                          >
                ...
                <div className='text-center py-6 md:py-8 text-muted-foreground'>
                  <Tag className='h-8 w-8 md:h-12 md:w-12 mx-auto mb-2 md:mb-3 opacity-50' />
                  <p className='text-xs md:text-sm'>Không có tùy chọn bổ sung</p>
                </div>
              )}
            </CardContent>
          </Card>

          <Card className='shadow-lg border-0 bg-gradient-to-br from-white to-orange-50 xl:col-span-1'>
            <CardHeader className='pb-3 md:pb-4'>
              <CardTitle className='flex items-center gap-2 text-base md:text-lg'>
                <Ruler className='h-4 w-4 md:h-5 md:w-5 text-orange-600' />
                Thông số đo cơ thể
              </CardTitle>
            </CardHeader>
            <CardContent className='space-y-3 md:space-y-4'>
              {measurement ? (
                ...
                <div className='text-center py-6 md:py-8 text-muted-foreground'>
                  <Ruler className='h-8 w-8 md:h-12 md:w-12 mx-auto mb-2 md:mb-3 opacity-50' />
                  <p className='text-xs md:text-sm'>Chưa có thông số đo cơ thể</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            {isLoadingAddress ? (
              ...
              <div className='space-y-3 md:space-y-4'>
                <div className='bg-green-50 p-3 md:p-4 rounded-lg border border-green-200'>
                  <p className='font-semibold text-gray-900 text-base md:text-lg mb-2'>{addressData.street}</p>
                  <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 text-xs md:text-sm'>
                    ...
                    <div className='flex flex-col sm:flex-row sm:items-center gap-1 md:gap-2 sm:col-span-2 lg:col-span-1'>
                ...
                <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
                  {addressData.isDefault && (
                    <Badge className='bg-blue-100 text-blue-700 border-blue-200 px-2 md:px-3 py-1 text-xs w-fit'>
              ...
              <div className='text-center py-6 md:py-8 text-muted-foreground'>
                <MapPin className='h-8 w-8 md:h-12 md:w-12 mx-auto mb-2 md:mb-3 opacity-50' />
                <p className='text-xs md:text-sm'>Không có thông tin địa chỉ</p>
              </div>
            )}
          </CardContent>
        </Card>
