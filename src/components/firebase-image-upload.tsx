...
export function FirebaseImageUpload({
  ...
  const { uploadMultiple, isConfigured } = useFirebaseUpload()
  ...
  const handleFileSelect = async (files: FileList | null) => {
    ...
    if (!isConfigured) {
      alert('Firebase chưa được cấu hình. Vui lòng kiểm tra Firebase configuration.')
      return
    }
    ...
    try {
      ...
      // Cập nhật images với URLs từ Firebase
      setImages((prev) => {
        const updatedImages = prev.map((img) => {
          if (img.isUploading && img.file) {
            ...
            if (fileIndex !== -1 && uploadResults[fileIndex]) {
              return {
                ...img,
                url: uploadResults[fileIndex].downloadURL,
                fullPath: uploadResults[fileIndex].fullPath,
                isUploading: false
              }
            }
          }
        ...
        // Gọi updateParent ngay lập tức với updatedImages
      ...
      setImages((prev) => {
        const failedImages = prev.filter((img) => !img.isUploading)
        updateParent(failedImages)
        return failedImages
      })
    }
  }
          ...
          {images.map((image) => (
            <div key={image.id} className='relative group'>
              <div className='aspect-square rounded-lg overflow-hidden border bg-gray-100'>
                ...
                {/* Firebase Badge */}
                {image.fullPath && !image.isUploading && (
