...
const NavBadge = ({ children }: { children: ReactNode }) => (
  <Badge 
    className='rounded-full px-1.5 py-0 text-[10px] font-bold bg-violet-100 text-violet-700 dark:bg-violet-900/50 dark:text-violet-300 border-violet-200 dark:border-violet-800'
  >
    {children}
  </Badge>
)
        ...
        <Link to={item.url} onClick={() => setOpenMobile(false)}>
          ...
          {item.icon && (
            <item.icon className={cn(
              ...
              isActive ? 'text-violet-600 dark:text-violet-400 scale-110' : 'group-hover:text-violet-500 group-hover:scale-105'
            )} />
          )}
                ...
                <SidebarMenuSubItem key={subItem.title}>
                  <SidebarMenuSubButton 
                    ...
                    className={cn(
                      ...
                      'hover:bg-violet-50 dark:hover:bg-violet-950/30',
                      isSubActive && 'bg-violet-100 dark:bg-violet-950/50 text-violet-900 dark:text-violet-100 font-semibold'
                    )}
                  >
          ...
          {item.items.map((sub) => (
            ...
            >
              <Link to={sub.url} className='flex items-center gap-2'>
                ...
                {sub.badge && <span className='text-xs font-bold text-violet-600 dark:text-violet-400'>{sub.badge}</span>}
              </Link>
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
